from dilate import soft_dtw2
from dilate import path_soft_dtw2
import torch, pickle
...
def dilate_loss(items, alpha=0.5, gamma=0.001):
    ...
    for i in range(len(items)):
        ...
        for j in range(i + 1, len(items)):
            Dk = soft_dtw2.pairwise_distances(
                items[i].reshape(-1, 1),
                items[j].reshape(-1, 1),
            )
            D[j - i - 1, :, :] = Dk
        ...
        paths = path_soft_dtw2.compute_dilate_path(D, gamma)

        tmp.ppp(f"{i}-> compute 1 \t")
        D = pickle.load(open("D.pkl", "rb"))
        loss_shapes = soft_dtw2.compute_soft_dtw_batch(D, gamma)
        ...
        for j in range(i + 1, len(items)):
            ...
            Omega = soft_dtw2.pairwise_distances(omega)
